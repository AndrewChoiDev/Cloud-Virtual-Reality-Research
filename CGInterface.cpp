...
bool ShaderOneInterface::PerSessionInit(CGInterface *cgi) {
  ...
  fragmentlmatrix = cgGetNamedParameter(fragmentProgram, "lmatrix");
  return true;
...
void ShaderOneInterface::PerFrameInit() {
	...
	// set intrinsics
	PPC sppc(*scene->ppc);
	sppc.TranslateRightLeft(-10.0f);
	sppc.SetIntrinsicsHW();
	// set extrinsics
	sppc.SetExtrinsicsHW();
	cgGLSetStateMatrixParameter(fragmentlmatrix,
